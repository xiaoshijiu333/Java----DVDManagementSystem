...
public class Adminoperation extends JFrame implements Runnable{
	...
	private JButton btn_admin_operation=null;
	private JButton btn_dvd_records=null;
	private JButton btn_exit=null;
	private Thread t=new Thread(this);
	private users u=null;
	
	public Adminoperation(users u){
		this.u=u;
		this.setTitle("管理员操作窗口");
		...
		btn_admin_operation=new JButton("管理员DVD操作");
		
		//给这个按钮添加监听器，让继承JInternalFrame的窗口依附在桌面面板上
		btn_admin_operation.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent arg0) {
				AdminDvdOperation admindvdo=new AdminDvdOperation();
				jp_desk.add(admindvdo);
				admindvdo.toFront();			//让它永远存在于最上层
			}
		});
		
		//让按钮中的哪个框框消失
		btn_admin_operation.setFocusPainted(false);
		...
		btn_dvd_records.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				AdminRecordOperation adrecOper=new AdminRecordOperation();
				jp_desk.add(adrecOper);
				adrecOper.toFront();
			}
		});
		...
		btn_exit.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				int flag=JOptionPane.showConfirmDialog(Adminoperation.this, "是否确定退出","确定信息", JOptionPane.YES_NO_OPTION);
				if(flag==JOptionPane.YES_OPTION){
					Adminoperation.this.dispose();
					new loginView();
				}				
			}
		});
		...
		jp_btn.add(new JPanel());
		jp_btn.add(btn_admin_operation);
