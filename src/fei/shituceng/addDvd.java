	...
	public addDvd(){
		...
		jb_confirm.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(dname.equals("")){
					JOptionPane.showMessageDialog(addDvd.this,"DVD名不能为空");
				}else if(count.equals("")){
					JOptionPane.showMessageDialog(addDvd.this,"借出次数不能为空");
				}else if(!dvdTool.isNumber(count)){
					JOptionPane.showMessageDialog(addDvd.this,"借出次数只能填写数字");
				}else{					
					int flag=JOptionPane.showConfirmDialog(addDvd.this, "是否确定添加DVD","确定信息", JOptionPane.YES_NO_OPTION);
					if(flag==JOptionPane.YES_OPTION){
						dvds d=new dvds(dname, Integer.valueOf(count), status);
						dyewu.adddvd(d);
						JOptionPane.showMessageDialog(addDvd.this, "添加成功！");
						addDvd.this.dispose();
					}else{
						JOptionPane.showMessageDialog(addDvd.this, "添加失败！");
						jt_name.setText("");
						jt_count.setText("");
					}
				}
		...
		jb_exit.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				int flag=JOptionPane.showConfirmDialog(addDvd.this, "是否确定退出","确定信息", JOptionPane.YES_NO_OPTION);
				if(flag==JOptionPane.YES_OPTION){
					addDvd.this.dispose();
				}					
			}
